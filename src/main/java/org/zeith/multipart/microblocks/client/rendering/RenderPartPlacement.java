...
public class RenderPartPlacement
{
	...
	public static void renderLast(RenderLevelStageEvent e)
	{
		...
		var pl = mc.player;
		var hit0 = mc.hitResult;
		if(pl == null || hit0 == null || !(hit0 instanceof BlockHitResult hit)) return;
		...
		VertexConsumer buf = mc.renderBuffers().crumblingBufferSource().getBuffer(RenderType.itemEntityTranslucentCull(InventoryMenu.BLOCK_ATLAS));
		...
		for(RenderType layer : RenderType.chunkBufferLayers())
			for(BakedQuad quad : ModelGeneratorSystem.generateMesh(type, placement, data, pc, mc.level, finalPos, strips, microstate, rng, layer).toBakedBlockQuads())
